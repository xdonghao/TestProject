...
@implementation EIBMActionSheetView
...
- (void)loadActionSheetUi
{
    ...
    if (_protext.length) {
        self.proL.text = _protext;
        [_backGroundView addSubview:self.proL];
    }
    
    for (NSInteger i = 0; i<_shareBtnTitleArray.count; i++) {
        EIBMVerButton *button = [EIBMVerButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(0, CGRectGetHeight(_proL.frame)+CancelBtnHeight*i, CGRectGetWidth(_backGroundView.frame), CancelBtnHeight);
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        _backGroundView.frame = CGRectMake(0, ActionSheetH-(_shareBtnTitleArray.count*CancelBtnHeight+CancelBtnHeight)-7-(_protext.length==0?0:45), ActionSheetW, _shareBtnTitleArray.count*CancelBtnHeight+CancelBtnHeight+7+(_protext.length==0?0:45));
    }];
...
- (void)loadUiConfig
{
    ...
    {
        EIBMActionButton *button = [EIBMActionButton buttonWithType:UIButtonTypeCustom];
        if (_shareBtnImgArray.count == 2) {
            button.frame = CGRectMake(_backGroundView.bounds.size.width/2*(i%2), AddTop+_LXActionSheetHeight+(i/2)*76, _backGroundView.bounds.size.width/2, 70);
        }else{
            if (_shareBtnImgArray.count%3 == 0) {
                button.frame = CGRectMake(_backGroundView.bounds.size.width/3*(i%3), AddTop+_LXActionSheetHeight+(i/3)*76, _backGroundView.bounds.size.width/3, 70);
            }
            else
            {
                button.frame = CGRectMake(_backGroundView.bounds.size.width/4*(i%4),AddTop+ _LXActionSheetHeight+(i/4)*76, _backGroundView.bounds.size.width/4, 70);
            }
        }
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        _backGroundView.frame = CGRectMake(ViewFrameX, ActionSheetH-CGRectGetHeight(_backGroundView.frame), ActionSheetW-2*ViewFrameX, CGRectGetHeight(_backGroundView.frame));
    }];
    ...
    if (_backGroundView == nil) {
        ...
        if (_showtype == ShowTypeIsShareStyle) {
            if (_shareBtnImgArray.count<5) {
                _backGroundView.frame = CGRectMake(ViewFrameX, ActionSheetH, ActionSheetW-2*ViewFrameX,AddTop+ 64+(_protext.length==0?0:45)+76+14);
            }else
            {
                NSInteger index;
                ...
                _backGroundView.frame = CGRectMake(ViewFrameX, ActionSheetH, ActionSheetW-ViewFrameX*2, AddTop+64+(_protext.length==0?0:45)+76*index+14);
            }
        }
        else
        {
            _backGroundView.frame = CGRectMake(0, ActionSheetH, ActionSheetW, _shareBtnTitleArray.count*CancelBtnHeight+CancelBtnHeight+7+(_protext.length==0?0:45));
...
- (UIView *)topsheetView
{
    if (_topsheetView == nil) {
        _topsheetView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_backGroundView.frame), CGRectGetHeight(_backGroundView.frame)-CancelBtnHeight)];
        _topsheetView.backgroundColor = [UIColor colorWithRed:0.92 green:0.92 blue:0.92 alpha:1.00];
...
{
    if (_proL == nil) {
        _proL = [[UILabel alloc] initWithFrame:CGRectMake(0, 5, CGRectGetWidth(_backGroundView.frame), 30)];
        _proL.text = @"分享至";
        _proL.textColor = [UIColor grayColor];
        _proL.textAlignment = NSTextAlignmentCenter;
        _proL.backgroundColor = [UIColor colorWithRed:0.92 green:0.92 blue:0.92 alpha:1.00];
    }
    return _proL;
}
...
- (UIButton *)cancelBtn
{
    if (_cancelBtn == nil) {
        ...
        if (_showtype == ShowTypeIsShareStyle) {
            _cancelBtn.frame = CGRectMake(0, CGRectGetHeight(_backGroundView.frame)-CancelBtnHeight, CGRectGetWidth(_backGroundView.frame), CancelBtnHeight);
