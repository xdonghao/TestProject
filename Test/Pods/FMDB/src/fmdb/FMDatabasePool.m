...
#endif
        if (success) {
            if ([self->_delegate respondsToSelector:@selector(databasePool:cc_shouldAddDatabaseToPool:)] && ![self->_delegate databasePool:self cc_shouldAddDatabaseToPool:db]) {
                [db close];
                db = 0x00;
            }
            else {
                ...
                if (![self->_databaseOutPool containsObject:db]) {
                    ...
                    if (shouldNotifyDelegate && [self->_delegate respondsToSelector:@selector(databasePool:cc_didAddDatabase:)]) {
                        [self->_delegate databasePool:self cc_didAddDatabase:db];
                    }
                }
            }
        }
